...
class SummarizationModel(object):
    """A class to represent a sequence-to-sequence model for text
    summarization. Supports both baseline mode, pointer-generator mode, and
    coverage"""
    ...
    def _add_placeholders(self):
        ...
        self._dec_batch = tf.placeholder(
            tf.int32, [hps.batch_size.value, hps.max_dec_steps], name='dec_batch'
        )
        self._target_batch = tf.placeholder(
            tf.int32, [hps.batch_size.value, hps.max_dec_steps],
            name='target_batch'
        )
        self._dec_padding_mask = tf.placeholder(
            tf.float32, [hps.batch_size.value, hps.max_dec_steps],
    ...
    def _reduce_states(self, fw_st, bw_st):
        ...
        with tf.variable_scope('reduce_final_st'):
            ...
            # Apply linear layer
            old_c = tf.concat(
                axis=1, values=[fw_st.c, bw_st.c]
            )  # Concatenation of fw and bw cell
            old_h = tf.concat(
                axis=1, values=[fw_st.h, bw_st.h]
    ...
    def _add_decoder(self, inputs):
        ...
        prev_coverage = self.prev_coverage if hps.mode.value == "decode" and \
            hps.coverage.value else None
    ...
    def _calc_final_dist(self, vocab_dists, attn_dists):
        ...
        with tf.variable_scope('final_distribution'):
            ...
            attn_dists = [
                (1 - p_gen) * dist for (p_gen, dist) in zip(self.p_gens, attn_dists)
            ]
            ...
            batch_nums = tf.range(
                0, limit=self._hps.batch_size.value
            )  # shape (batch_size)
            ...
            # to get the final distributions
            # final_dists is a list length max_dec_steps; each entry is a
            # tensor shape (batch_size, extended_vsize) giving the final
            # distribution for that decoder timestep
    ...
    def _add_emb_vis(self, embedding_var):
        ...
        https://www.tensorflow.org/get_started/embedding_viz Make the vocab
        metadata file, then make the projector config file pointing to it."""
        ...
        with tf.variable_scope('seq2seq'):
            ...
            with tf.variable_scope('decoder'):
                decoder_outputs, \
                    ...
                    self.attn_dists, \
                    self.p_gens, \
                    self.coverage = self._add_decoder(emb_dec_inputs)

            # Add the output projection to obtain the vocabulary distribution
            with tf.variable_scope('output_projection'):
                w = tf.get_variable(
                    'w', [hps.hidden_dim.value, vsize],
                    dtype=tf.float32,
                    initializer=self.trunc_norm_init
                )
                w_t = tf.transpose(w)
            ...
            if hps.mode.value in ['train', 'eval']:
                ...
                with tf.variable_scope('loss'):
                    ...
                    if hps.coverage.value:
                        ...
                        self._total_loss = self._loss + hps.cov_loss_wt.value * \
                            self._coverage_loss
    ...
    def run_encoder(self, sess, batch):
        ...
        Args:
          ...
          batch: Batch object that is the same example repeated across the
          batch (for beam search)
    ...
    def decode_onestep(
        ...
        Args:
          ...
          dec_init_states: List of beam_size LSTMStateTuples; the decoder
        ...
        Returns:
          ...
          attn_dists: List length beam_size containing lists length
          attn_length.
        ...
        return results['ids'], results['probs'
                                       ], new_states, attn_dists, p_gens, new_coverage
...
def _mask_and_avg(values, padding_mask):
    """Applies mask to values then returns overall average (a scalar)
      ...
      Args:
        values: a list length max_dec_steps containing arrays shape
        (batch_size).
        padding_mask: tensor shape (batch_size, max_dec_steps) containing 1s
        and 0s.
...
def _coverage_loss(attn_dists, padding_mask):
    """Calculates the coverage loss from the attention distributions.
      ...
      Args:
        attn_dists: The attention distributions for each decoder timestep. A
        list length max_dec_steps containing shape (batch_size, attn_length)
