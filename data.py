...
class Vocab(object):
    ...
    def __init__(self, vocab_file, max_size):
        '''Creates a vocab of up to max_size words, reading from the
        vocab_file. If max_size is 0, reads the entire vocab file.
        ...
        with open(vocab_file, 'r') as vocab_f:
            for line in vocab_f:
                ...
                if len(pieces) != 2:
                    print(
                        '''Warning: incorrectly formatted line in vocabulary
                        file: %s\n''' % line
                    )
                    continue
                w = pieces[0]
                ...
                if max_size != 0 and self._count >= max_size:
                    print(
                        '''max_size of vocab was specified as %i; we now have
                        %i words. Stopping reading.''' % (max_size, self._count)
                    )
                    break

        print(
            '''Finished constructing vocabulary of %i total words. Last word
            added: %s''' % (self._count, self._id_to_word[self._count - 1])
        )

    def word2id(self, word):
        '''Returns the id (integer) of a word (string). Returns [UNK] id if
        word is OOV.'''
    ...
    def write_metadata(self, fpath):
        ...
        described here: https://www.tensorflow.org/get_started/embedding_viz
...
def example_generator(data_path, single_pass):
    ...
    Binary data format:
    <length><blob>. <length> represents the byte size of <blob>. <blob> is
    serialized tf.Example proto. The tf.Example contains the tokenized article
    text and summary.

    Args:
        data_path: Path to tf.
        Example data files. Can include wildcards, e.g.  if you have several
        training data chunk files train_001.bin, train_002.bin, etc, then pass
        data_path=train_* to access them all.  single_pass: Boolean.  If True,
        go through the dataset exactly once, generating examples in the order
        they appear, then return. Otherwise, generate random examples
        indefinitely.

    Yields: Deserialized tf.Example.  '''
    while True:
    ...
    for w in article_words:
        ...
        if i == unk_id:  # If w is OOV
            if w not in oovs:  # Add to list of OOVs
                oovs.append(w)
                # This is 0 for the first article OOV, 1 for
                # the second article OOV...
            oov_num = oovs.index(w)
            # This is e.g. 50000 for the first article OOV,
            # 50001 for the second...
  ...
  '''
    ...
    for i in id_list:
        try:
            ...
            assert article_oovs is not None, '''Error: model produced a word ID
            that isn't in the vocabulary. This should not happen in baseline
            (no pointer-generator) mode'''
            article_oov_idx = i - vocab.size()
            try:
                w = article_oovs[article_oov_idx]

            # i doesn't correspond to an article oov
            except ValueError:
                raise ValueError(
