...
class Example(object):
    ...
    def get_dec_inp_targ_seqs(self, sequence, max_len, start_id, stop_id):
        ...
        input sequence for the decoder, and the target sequence which we will
        use to calculate loss. The sequence will be truncated if it is longer
        than max_len. The input sequence must start with the start_id and the
        target sequence must end with the stop_id (but not if it's been
        truncated).
...
class Batch(object):
    ...
    def init_encoder_seq(self, example_list, hps):
        ...
        self.enc_batch:
          ...
          ids (all OOVs represented by UNK id), padded to length of longest
        ...
        self.enc_padding_mask:
          numpy array of shape (batch_size, <=max_enc_steps), containing 1s and
        ...
        If hps.pointer_gen, additionally initializes the following:
          ...
          self.art_oovs:
            list of list of in-article OOVs (strings), for each example in
            the batch
    ...
    def init_decoder_seq(self, example_list, hps):
        """Initializes the following:
        self.dec_batch:
          numpy array of shape (batch_size, max_dec_steps), containing integer
          ids as input for the decoder, padded to max_dec_steps length.
        self.target_batch:
          numpy array of shape (batch_size, max_dec_steps), containing integer
          ids for the target sequence, padded to max_dec_steps length.
        self.dec_padding_mask:
          numpy array of shape (batch_size, max_dec_steps), containing 1s and
          0s. 1s correspond to real tokens in dec_batch and target_batch; 0s
          correspond to padding.
        """
    ...
    def next_batch(self):
        ...
        if self._batch_queue.qsize() == 0:
            tf.logging.warning(
                ...
                queue size: %i, Input queue size: %i''', self._batch_queue.qsize(),
    ...
    def fill_example_queue(self):
        ...
        while True:
            try:
                ...
                tf.logging.info(
                    """The example generator for this example queue filling
            ...
            example = Example(
                article, abstract_sentences, self._vocab, self._hps
    ...
    def text_generator(self, example_generator):
        ...
        while True:
            ...
            try:
                ...
                # data files
                abstract_text = e.features.feature['abstract'
