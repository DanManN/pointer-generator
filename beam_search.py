...
class Hypothesis(object):
    ...
    def __init__(self, tokens, log_probs, state, attn_dists, p_gens, coverage):
        ...
        Args:
          ...
          far.  coverage: Numpy array of shape (attn_length), or None if not
          using
...
def run_beam_search(sess, model, vocab, batch):
    ...
    # this will contain finished hypotheses (those that have emitted the [STOP]
    # token)
    results = []
    ...
    while steps < FLAGS.max_dec_steps and len(results) < FLAGS.beam_size:
        ...
        for i in range(num_orig_hyps):
            ...
            h, new_state, attn_dist, p_gen, new_coverage_i = hyps[i], new_states[
                i], attn_dists[i], p_gens[i], new_coverage[i]
