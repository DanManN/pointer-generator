...
tf.flags.DEFINE_boolean(
    ...
    fixed checkpoint, i.e. take the current checkpoint, and use it to produce
    one summary for each example in the dataset, write the summaries to file
    and then get ROUGE scores for the whole dataset. If False (default), run
    concurrent decoding, i.e. repeatedly load latest checkpoint, use it to
    produce summaries for randomly-chosen examples and log the results to
    screen, indefinitely.'''
)
...
tf.flags.DEFINE_string(
    'exp_name', '',
    '''Name for experiment. Logs will be saved in a directory with this name,
    under log_root.'''
)
...
tf.flags.DEFINE_integer(
    'min_dec_steps', 35,
    '''Minimum sequence length of generated summary. Applies only for beam
    search decoding mode'''
)
tf.flags.DEFINE_integer(
    'vocab_size', 50000,
    '''Size of vocabulary. These will be read from the vocabulary file in
    order. If the vocabulary file contains fewer words than this number, or if
    this number is set to 0, will take all words in the vocabulary file.'''
)
...
tf.flags.DEFINE_boolean(
    'coverage', False,
    '''Use coverage mechanism. Note, the experiments reported in the ACL paper
    ...
    turn this off for most of training then turn on for a short phase at the
    end.'''
)
tf.flags.DEFINE_float(
    'cov_loss_wt', 1.0,
    '''Weight of coverage loss (lambda in the paper). If zero, then no
    incentive to minimize coverage loss.'''
)

# Utility flags, for restoring and changing checkpoints
tf.flags.DEFINE_boolean(
    'convert_to_coverage_model', False,
    '''Convert a non-coverage model to a coverage model. Turn this on and run
    in train mode. Your current training model will be copied to a new version
    (same name with _cov_init appended) that will be ready to run with coverage
    flag turned on, for the coverage training stage.'''
)
tf.flags.DEFINE_boolean(
    'restore_best_model', False,
    '''Restore the best model in the eval/ dir and save it in the train/ dir,
    ready to be used for further training. Useful for early stopping, or if
    your training checkpoint has become corrupted with e.g. NaN values.'''
)
...
def calc_running_avg_loss(loss, running_avg_loss, summary_writer, step, decay=0.99):
    """Calculate the running average loss via exponential decay.  This is used
    to implement early stopping w.r.t. a more smooth loss curve than the raw
    loss curve.
...
def setup_training(model, batcher):
    ...
    if FLAGS.convert_to_coverage_model:
        assert FLAGS.coverage, """To convert your non-coverage model to a
        coverage model, run with convert_to_coverage_model=True and
        coverage=True"""
    ...
    try:
        run_training(
            model, batcher, sess_context_manager, sv, summary_writer
...
def run_eval(model, batcher, vocab):
    """Repeatedly runs eval iterations, logging to screen and writing
    summaries. Saves the model with the best loss seen so far."""
    model.build_graph()  # build the graph
...
def main(unused_argv):
    ...
    if hps.mode.value == 'train':
        ...
        # decode indefinitely (unless single_pass=True, in which case deocde
