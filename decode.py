...
evaluation and producing JSON datafiles for the in-browser attention
visualizer, which can be found here https://github.com/abisee/attn_vis"""
...
class BeamSearchDecoder(object):
    ...
    def decode(self):
        ...
        while True:
            ...
            if batch is None:  # finished decoding dataset in single_pass mode
                assert FLAGS.single_pass, """Dataset exhausted, but we are not
                in single_pass mode"""
                tf.logging.info("Decoder has finished reading dataset for single_pass.")
                tf.logging.info(
                    """Output has been saved in %s and %s. Now starting ROUGE
            ...
            if FLAGS.single_pass:
                ...
                if t1 - t0 > SECS_UNTIL_NEW_CKPT:
                    tf.logging.info(
                        ...
                        seconds. Time to load new checkpoint""", t1 - t0
                    )
                    _ = util.load_ckpt(self._saver, self._sess)
                    t0 = time.time()

    def write_for_rouge(self, reference_sents, decoded_words, ex_index):
        """Write output to file in correct format for eval with pyrouge. This
        is called in single_pass mode.
    ...
    def write_for_attnvis(self, article, abstract, decoded_words, attn_dists, p_gens):
        ...
        attention visualizer tool: https://github.com/abisee/attn_vis
...
def rouge_eval(ref_dir, dec_dir):
    ...
    r.system_filename_pattern = '(\d+)_decoded.txt'  # noqa
    r.model_dir = ref_dir
